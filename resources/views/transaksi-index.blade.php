        ...
        <tbody>
            @forelse ($transaksis as $t)
                <tr>
                    ...
                    <td class="px-4 py-2 text-center">
                        @if ($t->bukti_pembayaran)
                            <div x-data="{ open: false }">
                                <img src="{{ asset('bukti/' . $t->bukti_pembayaran) }}"
                                    class="w-16 h-16 object-cover rounded mx-auto cursor-pointer" @click="open = true">
                    ...
                    <td class="px-4 py-2 text-center"><a href="/transaksi/{{ $t->id }}/edit" class="text-blue-600">Edit</a>
                    </td>
                    <td class="border-y px-4 py-2 text-center">
                        <form action="/transaksi/{{ $t->id }}" method="POST" onsubmit="return confirm('Yakin mau hapus?')">
                            @csrf @method('DELETE')
